...
int *gen_indexes(int num_indexes, int rand_max, int min_seperation) {
  int not_done;
  int *indexes = malloc(sizeof(int) * num_indexes);
  do{
    not_done = 0;
...
void create_sequence(int *sequence, char validations[][32], int sequence_length, int num_validations, sim_matrix_t *simMatrix){
  int total_length = sequence_length;
  int end;
  for(int idx=0; idx < num_validations; idx++){
    total_length += strlen(validations[idx]);
  }
  int *indexes = gen_indexes(num_validations, total_length-32, 32);
...
seq_data_t *gen_scal_data( sim_matrix_t *simMatrix, int mainLen, int matchLen) {
  ...
  for(int idx=0; idx < 2; idx++){
    for(int jdx=0; jdx < 3; jdx++){
      validation_length = strlen(validations[idx][jdx]);
  ...
  new_scal_data->main = malloc(sizeof(codon_t)*(mainLen + validation_size));
  new_scal_data->match = malloc(sizeof(codon_t)*(matchLen + validation_size));
  

  int *gen_sequences[2] = {new_scal_data->main, new_scal_data->match};
  int seq_lengths[2] = {mainLen, matchLen};
